...
const dbController = {
  ...
  getAllPostsByUser: async (userEmail) => {
    try {
      ...
      const users = await dbController.getJSON(DB_PATH);
      const userPosts = users.flatMap((u) => u.posts).filter((post) => post.createdBy === userEmail);
      return userPosts;
    } catch (error) {
  ...
  getPostById: async (id) => {
    try {
      ...
      const post = posts.flatMap((user) => user.posts).find((post) => post.id === id);
      if (!post) {
  ...
  getPostByURL: async (url) => {
    try {
      const posts = await dbController.getJSON(DB_PATH);
      const post = posts.flatMap((user) => user.posts).find((post) => post.url === url);
      if (!post) {
        throw new Error('Post não encontrado');
      }
      return post;
